...
namespace Mod_Hub_Base
{
	...
	void Hooking::Start(HMODULE hmoduleDLL)
	{
		...
		Log::Msg(R"(
___________     ___.                          
\__    ___/____ \_ |__   _____ _____  ___  ___
  |    |  \__  \ | __ \ /     \\__  \ \  \/  /
  |    |   / __ \| \_\ \  Y Y  \/ __ \_>    < 
  |____|  (____  /___  /__|_|  (____  /__/\_ \
               \/    \/      \/     \/      \/
Join our discord server: https://discord.gg/unuAr8JhxQ
)");
	...
	{
		if (std::strcmp(label, "HUD_JOINING") == 0) return "Loading GTA Online with Gudgudak";
		if (std::strcmp(label, "HUD_TRANSP") == 0) return "Loading Singleplayer with Gudgudak";
		if (std::strcmp(label, "PM_EXIT_GAME") == 0) return "Exit GTAV with Gudgudak";
		if (std::strcmp(label, "PM_INF_EXIT") == 0) return "Exit to desktop with Gudgudak?";
		if (std::strcmp(label, "PM_ENTER_MP") == 0) return "Play GTA Online with Gudgudak";
		if (std::strcmp(label, "PM_INF_PGOB") == 0) return "Play GTA Online with Gudgudak, Gudgudak is currently undetected so no need to worry :)";
	...
	*/
	...
	void Hooking::FindPatterns()
	{
		...
		setPattern <Bytes24>("minecraftcheck",
		...
		Log::Msg(Mod_Menu_Name " <!> injected!");
		Log::Msg("Join our discord server: https://discord.gg/unuAr8JhxQ");
		Log::Msg("Our official website: https://gudgudakcheats.tk/");
		// system("start https://discord.gg/2gcz9EpBhN");
